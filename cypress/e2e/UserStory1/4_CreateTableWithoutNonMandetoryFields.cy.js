...
describe('4_CreateTableWithoutNonMandetoryFields', function ()  {
    before(() => {
...
let tableName=  Math.random().toString(36).substr(2, 8);
  helperObj.createTableWithoutOptionalFields(tableName,data.hashAttributeName,data.hashAttributeType,data.readCapacityUnits,data.writeCapacityUnits)
  assertionsValidations.validateDataExistSuccessfullyCreated(tableName)  //validate table created successfully
